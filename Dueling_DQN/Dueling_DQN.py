...
class DuelingNetwork(gluon.nn.Block):
    ...
    def forward(self, state):
        ...
        mean_advantage = (nd.sum(advantage, axis=1) / self.n_actions).detach().reshape((state.shape[0], 1))
        q_value = state_value + advantage - mean_advantage
        return q_value
...
class DuelingDQN:
    def __init__(self,
        ...
        # build the network
        self.target_network = DuelingNetwork(n_action)
        self.main_network = DuelingNetwork(n_action)
    ...
    def save_parameters(self):
        ...
        self.main_network.save_parameters('Dueling_DQN_main_network_parameters')

    def load_parameters(self):
        self.target_network.load_parameters('Dueling_DQN_target_network_parameters')
        self.main_network.load_parameters('Dueling_DQN_main_network_parameters')
...
if __name__ == '__main__':
    ...
    agent = DuelingDQN(n_action=env.action_space.n,
                       init_epsilon=1,
                       final_epsilon=0.1,
                       gamma=0.99,
                       buffer_size=3000,
